<div class="d-flex justify-content-center">
    <form method="POST" id="editPlayerForm" style="max-width: 950px; width:100%;">
        ...
        <input type="hidden" name="url" id="url" value="{{ route('players.update', $player->id) }}">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <input type="text" class="form-control py-2" id="firstname" name="firstname" value="{{ $player->firstname }}" placeholder="Enter first name">
                </div>
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <input type="text" class="form-control py-2" id="lastname" name="lastname" value="{{ $player->lastname }}" placeholder="Enter last name">
                </div>
                <div style="height:10px;">
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <input type="email" class="form-control py-2" id="email" name="email" value="{{ $player->email }}" placeholder="Enter email address">
                </div>
                <div style="height:10px;">
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <input type="tel" class="form-control py-2" id="phone" name="phone" value="{{ $player->phone }}" placeholder="Enter phone number">
                </div>
                <div style="height:10px;">
            ...
            <div class="col-md-6">
                ...
                <select class="form-select py-2" id="gender" name="gender">
                    <option value="" disabled {{ is_null($player->gender) ? 'selected' : '' }}>Select gender</option>
                    ...
                    @forelse ($playerBatches as $idx => $currentBatch)
                        <div class="card border border-secondary-subtle rounded-4 mb-3 p-3 assignment-row bg-white">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="fw-semibold text-secondary small">Assignment #<span class="row-num">{{ $idx + 1 }}</span></span>
                                <button type="button" class="btn btn-outline-danger btn-sm border-0 remove-assignment-btn {{ count($playerBatches) <= 1 ? 'd-none' : '' }}">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select form-select-sm sport-select" name="assignments[{{ $idx }}][sport_id]" required>
                                        ...
                                        @foreach ($sports as $sport)
                                            <option value="{{ $sport->id }}" {{ $currentBatch->sport_id == $sport->id ? 'selected' : '' }}>{{ $sport->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select form-select-sm level-select" name="assignments[{{ $idx }}][level_id]" required>
                                        ...
                                        @foreach ($currentBatch->sport->levels as $level)
                                            ...
                                            @endphp
                                            <option value="{{ $level->id }}" data-fees="{{ $fees }}" {{ $currentBatch->level_id == $level->id ? 'selected' : '' }}>{{ $level->name }} (Fees: ₹{{ $fees }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select form-select-sm batch-select" name="assignments[{{ $idx }}][batch_id]" required>
                                        ...
                                        @foreach ($batches as $b)
                                            <option value="{{ $b->id }}" {{ $currentBatch->id == $b->id ? 'selected' : '' }}>{{ $b->name }} ({{ $b->start_time }} - {{ $b->end_time }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
